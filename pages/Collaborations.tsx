        ...
        <div className="max-w-[1200px] mx-auto h-full p-6 flex flex-col gap-6 relative">
            ...
            <div id="collab-content">
                ...
                {activeTab === 'completed' && (
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        {[
                            { brand: "TechFlow AI", category: "SaaS", title: "Q3 Feature Highlights", logo: "https://lh3.googleusercontent.com/aida-public/AB6AXuAaaFbQihFfy-F6e5rien4N3nqD2ZVjagkT3ggFv5wZ2ytBjVxB3e5Lkw-2gUBg_mOc4f5FYiQ0Oyh0IQd8HYm49V1abSGN9so7_FUIO7nIMJVa4dg2k954krxoVARfLWHqET9tWAIrvxZViGNb290HSv5nG2vATLeKmVbexOJ6UPbuv0eQg3kUXQYHoRCJAbwj8L8L7ye9IqMX-sHBm0ehaYkW-GV4TYQsbW4xkfa1C1bLafHNqlfawtpnEGQsoYzRnMwznSWulJ-r", amount: "$1,200", date: "Oct 15, 2024" },
                            { brand: "UrbanStyle", category: "Fashion", title: "Summer Collection Haul", initials: "US", color: "bg-pink-50 dark:bg-pink-900/30 text-pink-700 dark:text-pink-400", amount: "$850", date: "Sep 28, 2024" },
                            { brand: "FitLife", category: "Health", title: "30-Day Fitness Journey", initials: "FL", color: "bg-green-50 dark:bg-green-900/30 text-green-700 dark:text-green-400", amount: "$2,400", date: "Sep 10, 2024" },
                        ].map((item, i) => (
                            <div key={i} className="bg-white dark:bg-gray-800 p-5 rounded-xl border border-border-color dark:border-gray-700 shadow-card hover:shadow-lg transition-all group cursor-pointer">
                                <div className="flex justify-between items-start mb-4">
                                    <div className="flex items-center gap-3">
                                        {item.logo ? (
                                            <div className="size-10 rounded-lg border border-gray-100 dark:border-gray-700 flex items-center justify-center bg-gray-50 dark:bg-gray-900 overflow-hidden">
                                                <img alt="Brand Logo" className="w-6 h-6 object-contain" src={item.logo} />
                                            </div>
                                        ) : (
                                            <div className={`size-10 rounded-lg flex items-center justify-center font-bold text-sm ${item.color}`}>
                                                {item.initials}
                                            </div>
                                        )}
                                        <div>
                                            <h3 className="text-sm font-bold text-text-primary dark:text-white">{item.brand}</h3>
                                            <p className="text-xs text-text-secondary dark:text-gray-400">{item.category}</p>
                                        </div>
                                    </div>
                                    <span className="px-2 py-1 bg-green-50 dark:bg-green-900/30 text-green-600 dark:text-green-400 text-[10px] uppercase font-bold tracking-wider rounded-md border border-green-100 dark:border-green-800">Completed</span>
                                </div>
                                <h4 className="text-base font-bold text-text-primary dark:text-white mb-3">{item.title}</h4>
                                <div className="flex items-center justify-between text-xs pt-3 border-t border-gray-100 dark:border-gray-700">
                                    <div className="flex items-center gap-1.5 text-green-600 dark:text-green-400 font-medium">
                                        ...
                                        <span>{item.amount} Paid</span>
                                    </div>
                                    <span className="text-text-secondary dark:text-gray-400">{item.date}</span>
                                </div>
                            </div>
                        ))}
                    </div>
