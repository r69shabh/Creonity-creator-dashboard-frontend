...
type Tab = 'active' | 'commissioned' | 'not_commissioned';
...
const Campaigns: React.FC = () => {
  ...
  const openProposalModal = (e: React.MouseEvent, title: string) => {
    e.preventDefault();
  ...
  return (
    <div className="max-w-[1200px] mx-auto flex flex-col gap-6 p-6">
      <div className="border-b border-gray-200 dark:border-gray-700">
        <nav aria-label="Tabs" className="-mb-px flex space-x-8 overflow-x-auto no-scrollbar">
          ...
          <button 
            ...
            className={`${activeTab === 'commissioned' ? 'border-primary text-primary' : 'border-transparent text-text-secondary dark:text-gray-400 hover:border-gray-300 dark:hover:border-gray-600 hover:text-text-primary dark:hover:text-gray-200'} whitespace-nowrap border-b-2 py-4 px-1 text-sm font-medium cursor-pointer bg-transparent focus:outline-none transition-colors shrink-0`}
          >
            Commissioned
            <span className={`ml-2 rounded-full px-2.5 py-0.5 text-xs font-medium ${activeTab === 'commissioned' ? 'bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-500' : 'bg-gray-100 dark:bg-gray-800 text-gray-900 dark:text-gray-400'}`}>2</span>
          </button>
          <button 
            onClick={() => setActiveTab('not_commissioned')}
            className={`${activeTab === 'not_commissioned' ? 'border-primary text-primary' : 'border-transparent text-text-secondary dark:text-gray-400 hover:border-gray-300 dark:hover:border-gray-600 hover:text-text-primary dark:hover:text-gray-200'} whitespace-nowrap border-b-2 py-4 px-1 text-sm font-medium cursor-pointer bg-transparent focus:outline-none transition-colors shrink-0`}
          >
            Not Commissioned
            <span className={`ml-2 rounded-full px-2.5 py-0.5 text-xs font-medium ${activeTab === 'not_commissioned' ? 'bg-red-100 dark:bg-red-900/30 text-red-700 dark:text-red-500' : 'bg-gray-100 dark:bg-gray-800 text-gray-900 dark:text-gray-400'}`}>2</span>
          </button>
        </nav>
      </div>
      ...
      <div className="flex flex-col gap-4">
          ...
          <>
            <Link to="/campaigns/vr-headset-review" className="bg-white dark:bg-gray-800 rounded-xl border border-border-color dark:border-gray-700 shadow-card hover:shadow-lg transition-all duration-200 group block">
              <div className="p-5 grid grid-cols-1 lg:grid-cols-12 gap-4 items-center">
                ...
                <div className="lg:col-span-2">
                  <p className="text-xs text-text-secondary dark:text-gray-400 lg:hidden mb-1">Budget</p>
                  <span className="text-sm font-bold text-text-primary dark:text-white">$500 - $800</span>
                </div>
                <div className="lg:col-span-2">
                ...
                <div className="lg:col-span-2 flex justify-start lg:justify-end gap-2 mt-2 lg:mt-0">
                  <button onClick={(e) => openProposalModal(e, 'VR Headset Review')} className="px-4 py-2 text-xs font-semibold text-text-primary dark:text-white bg-white dark:bg-gray-700 border border-gray-200 dark:border-gray-600 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-600 transition-colors z-10 relative">Bid Now</button>
            ...
            <Link to="/campaigns/morning-routine" className="bg-white dark:bg-gray-800 rounded-xl border border-border-color dark:border-gray-700 shadow-card hover:shadow-lg transition-all duration-200 group relative overflow-hidden block">
              ...
              <div className="p-5 grid grid-cols-1 lg:grid-cols-12 gap-4 items-center">
                ...
                <div className="lg:col-span-2">
                  <p className="text-xs text-text-secondary dark:text-gray-400 lg:hidden mb-1">Budget</p>
                  <span className="text-sm font-bold text-text-primary dark:text-white">$350 Fixed</span>
                </div>
                <div className="lg:col-span-2">
                  <p className="text-xs text-text-secondary dark:text-gray-400 lg:hidden mb-1">Ends In</p>
            ...
            <Link to="/campaigns/productivity-tool" className="bg-white dark:bg-gray-800 rounded-xl border border-border-color dark:border-gray-700 shadow-card hover:shadow-lg transition-all duration-200 group block">
              <div className="p-5 grid grid-cols-1 lg:grid-cols-12 gap-4 items-center">
                ...
                <div className="lg:col-span-2">
                  <p className="text-xs text-text-secondary dark:text-gray-400 lg:hidden mb-1">Budget</p>
                  <span className="text-sm font-bold text-text-primary dark:text-white">$1,500 - $2,500</span>
                </div>
                <div className="lg:col-span-2">
                  <p className="text-xs text-text-secondary dark:text-gray-400 lg:hidden mb-1">Ends In</p>
                  <div className="flex items-center gap-1.5 text-text-secondary dark:text-gray-400 font-medium text-sm">
        ...
        {/* ... (Existing Commissioned and Not Commissioned sections) ... */}
             ...
             {!isSuccess ? (
                 <>
                    <div className="px-6 py-4 border-b border-border-color dark:border-gray-700 bg-gray-50 dark:bg-gray-900/50 flex justify-between items-center">
                        <div>
                        <h3 className="font-bold text-text-primary dark:text-white">Submit Proposal</h3>
                        <p className="text-xs text-text-secondary dark:text-gray-400">For {selectedCampaign}</p>
                        </div>
                        <button onClick={closeModals} className="text-text-secondary hover:text-text-primary dark:text-gray-400 dark:hover:text-white"><span className="material-symbols-outlined">close</span></button>
                    </div>
                    <div className="p-6 flex flex-col gap-4">
                        <div>
                            <label className="block text-xs font-bold text-text-primary dark:text-gray-300 uppercase tracking-wide mb-1.5">Proposed Rate ($)</label>
                        ...
                        <div>
                            <label className="block text-xs font-bold text-text-primary dark:text-gray-300 uppercase tracking-wide mb-1.5">Delivery Time</label>
                            <select className="w-full px-4 py-2.5 rounded-lg border border-border-color dark:border-gray-700 bg-white dark:bg-gray-900 text-text-primary dark:text-white focus:ring-primary focus:border-primary text-sm">
                 ...
                 <div className="p-8 flex flex-col items-center text-center">
                     ...
                     <h3 className="text-xl font-bold text-text-primary dark:text-white mb-2">Proposal Sent!</h3>
                     <p className="text-text-secondary dark:text-gray-400 text-sm mb-6">Your proposal for <strong>{selectedCampaign}</strong> has been successfully submitted to the brand. Good luck!</p>
                     <button onClick={closeModals} className="w-full py-2.5 bg-gray-100 dark:bg-gray-700 text-text-primary dark:text-white font-semibold rounded-lg hover:bg-gray-200 dark:hover:bg-gray-600 transition-colors">
                         Close
                     </button>
                 </div>
             )}
